...
class Rest{    
    ...
    public function updateWisata($wstData){   
        if($wstData["id"]) {     
            ...
            $wstQuery=" UPDATE ".$this->wstTable." SET kota='".$wstKota."', landmark='".$wstLandmark."', tarif='".$wstTarif."'
    ...
    public function deleteWisata($wstId) { 
        if($wstId) { 
            $wstQuery = " DELETE FROM ".$this->wstTable." WHERE id_wisata = '".$wstId."' ORDER BY id_wisata DESC";    
            mysqli_query($this->dbConnect, $wstQuery);    
            if(mysqli_affected_rows($this->dbConnect) > 0) { 
